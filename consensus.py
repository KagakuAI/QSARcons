...
class HyperoptSearchRegressor:
    ...
    def run(self, x, y):
        ...
        fspace = {}
        for i in range(1, self.cons_size + 1):
            fspace[f'M{i}'] = hp.choice(f'M{i}', range(len(x.columns)))

        def objective(params):

            hp_cons = [params[f"M{i}"] for i in range(1, self.cons_size + 1)]
            y_cons = x[x.columns[hp_cons]].mean(axis=1)
        ...
        #
        trials = Trials()
        best = fmin(fn=objective, space=fspace, algo=tpe.suggest, max_evals=self.n_iter, trials=trials, verbose=False)
        #
        best_cons = [i.item() for i in best.values()]
        best_cons = x.columns[best_cons]
        #
        return best_cons
